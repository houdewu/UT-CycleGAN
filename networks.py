...
class UnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, num_downs, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False):
        ...
        def conv_block(in_channels, out_channels, norm_layer, use_dropout):
            # Depthwise Convolution
            block = [nn.Conv2d(in_channels, in_channels, kernel_size=4, stride=2, padding=1, groups=in_channels)]
            # Pointwise Convolution
            block += [nn.Conv2d(in_channels, out_channels, kernel_size=1, stride=1, padding=0)]
            block += [norm_layer(out_channels)]
            block += [nn.ReLU(True)]
            if use_dropout:
        ...
        unet_block = UnetSkipConnectionBlock(ngf * 16, ngf * 16, input_nc=None, submodule=None, norm_layer=norm_layer,
                                             innermost=True)
...
class UnetSkipConnectionBlock(nn.Module):
    def __init__(self, outer_nc, inner_nc, input_nc=None,
                 submodule=None, outermost=False, innermost=False, norm_layer=nn.BatchNorm2d, use_dropout=False, use_transformer=True):
